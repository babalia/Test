...
var param = {
			...
			,ip:[/((?:(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d))))/,"请输入正确的ip"]
			,table:[/^[A-Za-z0-9_]*$/g,"请输入数字字母或者下划线"]
	...
	// 获取所有input
function check2(id){
	$("#"+id).children("li").children("input").each(function(index ,value){
	...
	$("#"+id).children("li").children(".sel_menu2").each(function(index, value){
		$(value).on("select2:close", function (e) { 
			checkSelect2($(this),index);
		});
	});
	$("#"+id).children("li").children(".sel_menu").each(function(index, value){
		$(value).on("select2:close", function (e) { 
...
function checkSelect2(select, index){
